...
export function registerInboxTools(server: McpServer, env: Env): void {
  server.registerTool(
    "whatsmcp_history_list",
    {
      ...
      description:
        "List webhook events stored by WhatsMCP (inbound messages, messages sent by this number when notify-sent-by-me is on, delivery, status, presence, connect/disconnect). fromMe distinguishes ours vs theirs. Use this to read a conversation. zapi_chats_get does not return message text.",
    ...
    async ({ phone, kind, fromMe, since, limit }) => {
      const events = await listEvents(env.DB, { phone, kind, fromMe, since, limit });
      return jsonResult(events.map((event) => eventForClient(event, env)));
    },
  );
  ...
  server.registerTool(
    "whatsmcp_history_search",
    {
      ...
      description:
        "Full-text search over stored webhook text, chat name, sender, and phone. Optional phone/kind filters.",
      inputSchema: z.object({
        query: z.string(),
        phone: z.string().optional(),
        kind: z.enum(WEBHOOK_KINDS).optional(),
    ...
    async ({ query, phone, kind, limit }) => {
      const events = await searchEvents(env.DB, { query, phone, kind, limit });
      return jsonResult(events.map((event) => eventForClient(event, env)));
    },
  );
